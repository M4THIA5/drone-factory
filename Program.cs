    ...
    try
    {
        switch (command)
        {
            ...
            case "PRODUCE":
                ...
                break;
            case "ADD_TEMPLATE":
                HandleAddTemplate(commandArgs, stock);
                break;
            default:
...
static void HandleAddTemplate(string args, Stock stock)
{
    ...
    var recipe = new DroneRecipe(
        byKind[PieceKind.Hull],
        byKind[PieceKind.Core],
        byKind[PieceKind.Generator],
        byKind[PieceKind.Move],
        byKind[PieceKind.Processor],
        byKind[PieceKind.System]);

    Categorizer.Validate(recipe);

    Catalogue.Drones[name] = recipe;
    stock.AddDrone(name, 0);
